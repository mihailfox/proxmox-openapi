...
export function resolveAutomationPipelineOptions(options: AutomationPipelineRunOptions = {}): PipelineOptions {
  ...
  const rawSnapshotPath = path.resolve(
    options.rawSnapshotPath ?? "tools/api-scraper/data/raw/proxmox-openapi-schema.json"
  );
  const irOutputPath = path.resolve(options.irOutputPath ?? "tools/api-normalizer/data/ir/proxmox-openapi-ir.json");
